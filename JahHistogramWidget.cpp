...
JahHistogramWidget::JahHistogramWidget(QWidget *parent)
    : QWidget(parent)
    , m_levels(128)
    , m_isBusy(false)
{
...
JahHistogramWidget::~JahHistogramWidget()
{
    m_processorThread.quit();
    m_processorThread.wait(10000);
}

void JahHistogramWidget::processFrame(QVideoFrame frame)
{
    if (m_isBusy)
        return; //drop frame
...
void JahHistogramWidget::setHistogram(QVector<qreal> histogram)
{
    m_isBusy = false;
...
void JahHistogramWidget::paintEvent(QPaintEvent *event)
{
    Q_UNUSED(event);
...
void JahFrameProcessor::processFrame(QVideoFrame frame, int levels)
{
